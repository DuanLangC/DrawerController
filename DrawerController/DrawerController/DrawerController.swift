...
public protocol DrawerControllerDelegate: class{
    /**
    An optional delegation method that is fired before the DrawerController opens.
    */
    func drawerController(drawerController: DrawerController, willOpen position: DrawerPosition)

    /**
     An optional delegation method that is fired after the DrawerController opened.
     */
    func drawerController(drawerController: DrawerController, didOpen position: DrawerPosition)

    /**
     An optional delegation method that is fired before the DrawerController closes.
     */
    func drawerController(drawerController: DrawerController, willClose position: DrawerPosition)

    /**
     An optional delegation method that is fired after the DrawerController closed.
     */
    func drawerController(drawerController: DrawerController, didClose position: DrawerPosition)

    /**
     An optional delegation method that is fired when the DrawerController pan gesture begins.
     */
    func drawerController(drawerController: DrawerController, didBeginPanAt point: CGPoint, position: DrawerPosition)

    /**
     An optional delegation method that is fired when the DrawerController pan gesture changes position.
     */
    func drawerController(drawerController: DrawerController, didChangePanAt point: CGPoint, position: DrawerPosition)

    /**
     An optional delegation method that is fired when the DrawerController pan gesture ends.
     */
    func drawerController(drawerController: DrawerController, didEndPanAt point: CGPoint, position: DrawerPosition)

    /**
     An optional delegation method that is fired when the DrawerController tap gesture executes.
     */
    func drawerController(drawerController: DrawerController, didTapAt point: CGPoint, position: DrawerPosition)
...
open class DrawerController : TransitionController{
    //Indicating if the panel is animating
    fileprivate var isAnimating = false

    //Used internally to track the original (x) position of container view when panning
    fileprivate var originalX: CGFloat = 0

    //Used for the leftView pan gesture
    fileprivate(set) var leftPanGesture: UIPanGestureRecognizer?

    //Used for the leftView tap gesture
    fileprivate(set) var leftTapGesture: UITapGestureRecognizer?

    //Used for the rightView pan gesture
    fileprivate(set) var rightPanGesture: UIPanGestureRecognizer?

    //Used for the rightView tap gesture
    fileprivate(set) var rightTapGesture: UITapGestureRecognizer?
    ...
    */
    ...
    /**
     Used to bind the delegation object.
    */
    open weak var delegate: DrawerControllerDelegate?
    /**
    ...
    */
    ...
    /**
     A Boolean property that enables and disables the leftView and rightView from
 ...
 */
    ...
    /**
     An initializer for the DrawerController.
...
//                    v.isShadowPathAutoSizing = true
    ...
    */
    ...
    open func openLeftView(velocity: CGFloat = 0) {
        ...
        delegate?.drawerController(drawerController: self, willOpen: .left)
        ...
        UIView.animate(withDuration: TimeInterval(0 == velocity ? animationDuration : fmax(0.1, fmin(1, Double(v.frame.origin.x / velocity)))),
            ...
            self.delegate?.drawerController(drawerController: self, didOpen: .left)
        }
    }
    ...
    open func openRightView(velocity: CGFloat = 0) {
        ...
        delegate?.drawerController(drawerController: self, willOpen: .right)
        ...
        UIView.animate(withDuration: TimeInterval(0 == velocity ? animationDuration : fmax(0.1, fmin(1, Double(v.frame.origin.x / velocity)))),
            ...
            self.delegate?.drawerController(drawerController: self, didOpen: .right)
        }
    }
    ...
    open func closeLeftView(velocity: CGFloat = 0) {
        ...
        delegate?.drawerController(drawerController: self, willClose: .left)
        ...
        UIView.animate(withDuration: TimeInterval(0 == velocity ? animationDuration : fmax(0.1, fmin(1, Double(v.frame.origin.x / velocity)))),
            ...
            self.delegate?.drawerController(drawerController: self, didClose: .left)
        }
    }
    ...
    open func closeRightView(velocity: CGFloat = 0) {
        ...
        delegate?.drawerController(drawerController: self, willClose: .right)
        ...
        UIView.animate(withDuration: TimeInterval(0 == velocity ? animationDuration : fmax(0.1, fmin(1, Double(v.frame.origin.x / velocity)))),
            ...
            self.delegate?.drawerController(drawerController: self, didClose: .right)
        }
    }
    ...
    fileprivate func showStatusBar() {
        DispatchQueue.main.async(execute: {[weak self] in
            ...
            self.delegate?.drawerController(drawerController: self, statusBar: false)
        })
    }
    ...
    fileprivate func hideStatusBar() {
        ...
        DispatchQueue.main.async(execute: {[weak self] in
            ...
            self.delegate?.drawerController(drawerController: self, statusBar: true)
        })
    }
...
extension DrawerController {
    ...
    fileprivate func prepareRightView() {
        ...
        rightViewWidth = .phone == UIDevice.current.userInterfaceIdiom ? 280 : 320
        rightView = UIView()
...
extension DrawerController: UIGestureRecognizerDelegate {
    ...
    fileprivate func handleLeftViewPanGesture(recognizer: UIPanGestureRecognizer) {
        ...
        case .began:
            ...
            delegate?.drawerController(drawerController: self, didBeginPanAt: point, position: .left)
        ...
        case .changed:
            ...
            let a = 1 - (v.layer.position.x + (w/2)) / v.bounds.width
            ...
            delegate?.drawerController(drawerController: self, didChangePanAt: point, position: .left)

        case .ended, .cancelled, .failed:
            let p = recognizer.velocity(in: recognizer.view)
            let x = p.x >= 1000 || p.x <= -1000 ? p.x : 0

            delegate?.drawerController(drawerController: self, didEndPanAt: point, position: .left)
    ...
    fileprivate func handleRightViewPanGesture(recognizer: UIPanGestureRecognizer) {
        ...
        case .began:
            ...
            delegate?.drawerController(drawerController: self, didBeginPanAt: point, position: .right)
        ...
        case .changed:
            ...
            delegate?.drawerController(drawerController: self, didChangePanAt: point, position: .right)

        case .ended, .cancelled, .failed:
            let p = recognizer.velocity(in: recognizer.view)
            let x = p.x >= 1000 || p.x <= -1000 ? p.x : 0

            delegate?.drawerController(drawerController: self, didEndPanAt: point, position: .right)
    ...
    fileprivate func handleLeftViewTapGesture(recognizer: UITapGestureRecognizer) {
        ...
        delegate?.drawerController(drawerController: self, didTapAt: recognizer.location(in: view), position: .left)
    ...
    fileprivate func handleRightViewTapGesture(recognizer: UITapGestureRecognizer) {
        ...
        delegate?.drawerController(drawerController: self, didTapAt: recognizer.location(in: view), position: .right)
